...
")
  ...
  foreach($line in @(@($raw))){
    if(-not [string]::IsNullOrWhiteSpace($line)){ [void]$res.Add($line.Trim()) }
  }
  return $res.ToArray()
}
...
function Get-LabelValue($Labels,[string]$Key){
  ...
  $p = $Labels.PSObject.Properties[$Key]
...
function Assert-ProjectLabel([string]$Kind,[string]$Name,$Labels){
  ...
  if([string]::IsNullOrWhiteSpace($project)){ Die "LABEL_PROJECT_MISSING" ($Kind + ":" + $Name) }
  if($project -ne $ComposeProject){ Die "LABEL_PROJECT_MISMATCH" ($Kind + ":" + $Name + ": expected=" + $ComposeProject + " got=" + $project) }
}
...
if(@($psLines).Count -lt 1){ Die "PMP_CONTAINERS_MISSING" ("project=" + $ComposeProject) }
$found = @{}
foreach($ln in @(@($psLines))){
  ...
  $ports = $parts[2]
  $image = $parts[3]
  $found[$name] = [ordered]@{ name=$name; status=$status; ports=$ports; image=$image }
}

foreach($required in @(@($DbContainer,$ApiContainer))){
  if(-not $found.ContainsKey($required)){ Die "CONTAINER_NOT_FOUND" $required }
}
foreach($k in @(@($found.Keys))){
  if(($k -ne $DbContainer) -and ($k -ne $ApiContainer)){ Die "EXTRA_PMP_CONTAINER" $k }
}
...
if(([string]$db.ports) -notmatch [regex]::Escape($needDb)){ Die "DB_PORT_MISMATCH" ("need=" + $needDb + " got=" + [string]$db.ports) }
if(([string]$api.ports) -notmatch [regex]::Escape($needApi)){ Die "API_PORT_MISMATCH" ("need=" + $needApi + " got=" + [string]$api.ports) }

$netJson = Run-Docker @("network","inspect",$NetworkName,"--format","{{json .Labels}}")
$netLine = (To-Lines $netJson | Select-Object -First 1)
try{ $netLabels = $netLine | ConvertFrom-Json -ErrorAction Stop } catch { Die "NETWORK_LABELS_JSON_BAD" ($NetworkName + ":" + $netLine) }
...
try{ $volLabels = $volLine | ConvertFrom-Json -ErrorAction Stop } catch { Die "VOLUME_LABELS_JSON_BAD" ($DbVolume + ":" + $volLine) }
...
try{ $mounts = $mountsLine | ConvertFrom-Json -ErrorAction Stop } catch { Die "DB_MOUNTS_JSON_BAD" ($DbContainer + ":" + $mountsLine) }
$okMount = $false
foreach($m in @(@($mounts))){
  $t = [string]$m.Type
  $n = [string]$m.Name
  $d = [string]$m.Destination
  if(($t -eq "volume") -and ($n -eq $DbVolume) -and ($d -eq "/var/lib/postgresql/data")){ $okMount = $true }
}
if(-not $okMount){ Die "DB_VOLUME_MOUNT_MISMATCH" ("need=" + $DbVolume + " to /var/lib/postgresql/data") }
